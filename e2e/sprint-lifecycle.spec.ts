...
test('plans, starts, executes, and completes a sprint across Backlog and Board', async ({
  ...
  await expect(page.getByText('Define MVP acceptance criteria')).toBeVisible();
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        ).sprints.find((sprint) => sprint.name === 'Sprint 2 delivery');
      }),
    )
    .toMatchObject({
      status: 'active',
...
test('edits and deletes a planned sprint without losing its work', async ({
  ...
  await page
    .getByRole('button', { name: 'Delete Renamed planned sprint' })
    .click();
