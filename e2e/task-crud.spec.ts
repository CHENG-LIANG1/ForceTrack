...
test('creates, edits, persists, and deletes a task', async ({ page }) => {
  ...
  await createDialog.getByRole('textbox', { name: /Title/ }).fill('E2E task');
  await createDialog
    .getByRole('combobox', { name: 'Status' })
    .selectOption('in_progress');
  await createDialog.getByLabel('Start date').fill('2026-08-11');
  await createDialog.getByLabel('Due date').fill('2026-08-13');
